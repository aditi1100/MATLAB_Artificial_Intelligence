function [label,imagedata,irows,icols] = readUByteImageAndLabel(index, image_filename, label_filename)
%
...
%magic_number
%number_of_items
count=fseek(myfile,index,'cof');
[label,count] = fread(myfile, 1, 'uint8', 0, 'ieee-be');
if (count==-1)
...
%number_rows
%number_columns
count=fseek(myfile,index*number_rows*number_columns,'cof');
[pixels,count] = fread(myfile, number_rows*number_columns, 'uint8', 0, 'ieee-be');
if (count==-1)
  fprintf(1,'ERROR: Could not read all image pixel values from file %s.\n', image_filename);
  exit(0);
end
imagedata = reshape( pixels, number_columns, number_rows)';
